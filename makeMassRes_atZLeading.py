...
sampleLists  = {
	...
	"2016PtBinned":["dileptonAna_resolution_2016_dyPt0To50_UL2016.root","dileptonAna_resolution_2016_dyPt50To100_UL2016.root","dileptonAna_resolution_2016_dyPt100To250_UL2016.root","dileptonAna_resolution_2016_dyPt250To400_UL2016.root","dileptonAna_resolution_2016_dyPt400To650_UL2016.root","dileptonAna_resolution_2016_dyPt650ToInf_UL2016.root"],
	"2016HIPMPtBinned":["dileptonAna_resolution_2016_dyPt0To50_UL2016HIPM.root","dileptonAna_resolution_2016_dyPt50To100_UL2016HIPM.root","dileptonAna_resolution_2016_dyPt100To250_UL2016HIPM.root","dileptonAna_resolution_2016_dyPt250To400_UL2016HIPM.root","dileptonAna_resolution_2016_dyPt400To650_UL2016HIPM.root","dileptonAna_resolution_2016_dyPt650ToInf_UL2016HIPM.root"],
	"2017PtBinned":["dileptonAna_resolution_dyPt0To50_UL2017.root","dileptonAna_resolution_dyPt50To100_UL2017.root","dileptonAna_resolution_dyPt100To250_UL2017.root","dileptonAna_resolution_dyPt250To400_UL2017.root","dileptonAna_resolution_dyPt400To650_UL2017.root","dileptonAna_resolution_dyPt650ToInf_UL2017.root"],
	"2018PtBinned":["dileptonAna_resolution_2018_dyPt0To50_UL2018.root","dileptonAna_resolution_2018_dyPt50To100_UL2018.root","dileptonAna_resolution_2018_dyPt100To250_UL2018.root","dileptonAna_resolution_2018_dyPt250To400_UL2018.root","dileptonAna_resolution_2018_dyPt400To650_UL2018.root","dileptonAna_resolution_2018_dyPt650ToInf_UL2018.root"],
...
xSecs = {
	...
	"2016HIPMPtBinned": [1485,354.3,83.12,3.047,0.3921,0.03636],
	"2016PtBinned": [1485,354.3,83.12,3.047,0.3921,0.03636],
	"2017PtBinned": [1485,354.3,83.12,3.047,0.3921,0.03636],
	"2018PtBinned": [1485,354.3,83.12,3.047,0.3921,0.03636],
...
def getBinRange(histo,mlow,mhigh,reg="Barrel"):
	ymin =  0
	ymax = -1
	...
	if mhigh==ptbins[len(ptbins)-2] and "Endcap" in reg: ymax=-1
	return ymin,ymax
...
def loadHistos(inputdata,inputMC,region,weights,weights2,trackType,mcIsData,dataIsMC):
	...
	reg = ""
	if   ("BB" in region or "Barrel" in region):  reg = "Barrel"
	elif ("BE" in region or region == "Overlap"):  reg = "Overlap"
	elif ("BE" in region or region == "Endcap"):  reg = "Endcap"
	elif ("BE" in region or region == "OverlapEndcap"):  
		reg1 = "Overlap"
		reg2 = "Endcap"
		
	if region == "OverlapEndcap":
		if not dataIsMC:
			hdata = _fileDATA[0].Get("Our2017MuonsPlusMuonsMinus%sResolution/DileptonMass_2d_vsLeadingPt_EtaMatched_%s" %(trackType,"Overlap")).Clone()
			hdata.Add(_fileDATA[0].Get("Our2017MuonsPlusMuonsMinus%sResolution/DileptonMass_2d_vsLeadingPt_EtaMatched_%s" %(trackType,"Endcap")).Clone())
		else:	
			for k,data in enumerate(inputdata):
				tmp   = _fileDATA[k].Get("Our2017MuonsPlusMuonsMinus%sResolution/DileptonMass_2d_vsLeadingPt_EtaMatched_%s" %(trackType,reg)).Clone()
		...
		if mcIsData:
			hmc = _fileMC[0].Get("Our2017MuonsPlusMuonsMinus%sResolution/DileptonMass_2d_vsLeadingPt_EtaMatched_%s" %(trackType,reg)).Clone()
		else:	
			for k,mc in enumerate(inputMC):
				#print mc
				tmp   = _fileMC[k].Get("Our2017MuonsPlusMuonsMinus%sResolution/DileptonMass_2d_vsLeadingPt_EtaMatched_%s" %(trackType,"Overlap")).Clone()
				tmp2  = _fileMC[k].Get("Our2017MuonsPlusMuonsMinus%sResolution/DileptonMass_2d_vsLeadingPt_EtaMatched_%s" %(trackType,"Endcap")).Clone()
				...
				if k==0 and not weights: 
					...
					tmp2.Scale(40000*weights[k]/nEvents)
					hmc = tmp
					hmc.Add(tmp2)
				elif not weights:
					hmc.Add(tmp)
		...
		if not dataIsMC:
			hdata = _fileDATA[0].Get("Our2017MuonsPlusMuonsMinus%sResolution/DileptonMass_2d_vsLeadingPt_EtaMatched_%s" %(trackType,reg)).Clone()
		else:	
			for k,data in enumerate(inputdata):
				tmp   = _fileDATA[k].Get("Our2017MuonsPlusMuonsMinus%sResolution/DileptonMass_2d_vsLeadingPt_EtaMatched_%s" %(trackType,reg)).Clone()
				if k==0 and not weights2: 
		...
		if mcIsData:
			...
			for k,mc in enumerate(inputMC):
				#print mc
				tmp   = _fileMC[k].Get("Our2017MuonsPlusMuonsMinus%sResolution/DileptonMass_2d_vsLeadingPt_EtaMatched_%s" %(trackType,reg)).Clone()
...
def drawMassRes(data,mc,output,rapidity,ptda,ptmc,trackType,funct,mcIsData,dataIsMC,year):
	...
	res_mc.GetXaxis().SetTitle("Leading p_{T} (#mu^{#pm}) (GeV)")
	res_mc.GetYaxis().SetTitleOffset(1.5)
	res_mc.Draw("P E0 SAME")
	if rapidity == "Barrel": leg = TLegend(0.5,0.65,0.95,0.90,"muon |#eta| < 1.2","brNDC")
	elif rapidity == "Overlap": leg = TLegend(0.5,0.65,0.95,0.9,"1.2 < muon |#eta| < 1.6","brNDC")
	elif rapidity == "Endcap": leg = TLegend(0.5,0.65,0.95,0.9,"muon |#eta| > 1.6","brNDC")
	elif rapidity == "OverlapEndcap": leg = TLegend(0.5,0.65,0.95,0.9,"muon |#eta| > 1.2","brNDC")
	...
	if '2016HIPM' in year:
		latex.DrawLatexNDC(0.50, 0.96, "#scale[0.8]{#font[42]{       2016 pre-VFP, 19.3 fb^{-1} (13 TeV)}}")
	if '2016' in year:
		latex.DrawLatexNDC(0.50, 0.96, "#scale[0.8]{#font[42]{       2016, 17.0 fb^{-1} (13 TeV)}}")
	elif '2017' in year:	
...
def makeMassRes(inputDATA,inputMC,output,weights,weights2,trackType,funct,mcIsData,dataIsMC):
	...
	(data_B,mc_B,ptdaB,ptmcB) = loadHistos(inputDATA,inputMC,"Barrel",weights,weights2,trackType,mcIsData,dataIsMC)
	(data_O,mc_O,ptdaO,ptmcO) = loadHistos(inputDATA,inputMC,"Overlap",weights,weights2,trackType,mcIsData,dataIsMC)
	(data_E,mc_E,ptdaE,ptmcE) = loadHistos(inputDATA,inputMC,"Endcap",weights,weights2,trackType,mcIsData,dataIsMC)
	(data_OE,mc_OE,ptdaOE,ptmcOE) = loadHistos(inputDATA,inputMC,"OverlapEndcap",weights,weights2,trackType,mcIsData,dataIsMC)
	# ~ (data_E2,mc_E2,ptdaE2,ptmcE2) = loadHistos(inputDATA,inputMC,"BE_neweta",weights,weights2,trackType,mcIsData,dataIsMC)

	drawMassRes(data_B,mc_B,output,"Barrel",ptdaB,ptmcB,trackType,funct,mcIsData,dataIsMC,inputDATA)
	drawMassRes(data_O,mc_O,output,"Overlap",ptdaO,ptmcO,trackType,funct,mcIsData,dataIsMC,inputDATA)
	drawMassRes(data_E,mc_E,output,"Endcap",ptdaE,ptmcE,trackType,funct,mcIsData,dataIsMC,inputDATA)
	drawMassRes(data_OE,mc_OE,output,"OverlapEndcap",ptdaOE,ptmcOE,trackType,funct,mcIsData,dataIsMC,inputDATA)
