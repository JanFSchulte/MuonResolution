...
mrange = [120, 200, 300, 400, 600, 800, 1000, 1300, 1600, 2000, 2500, 3000]
...
def doFitGeneric(hist,output,rap="BB",fit="cruijff",syst=False):
	...
	for i,h in enumerate(hist):
		...
		if ("cruijff" in fit or "doubleCB" in fit or "gaussExp" in fit):
...
#            funct.SetParLimits(1, gaus.GetParameter(1)*0.5, gaus.GetParameter(1)*1.5)
		...
		h.SetTitle("p resolution for %d < p <%d" %(mrange[i],mrange[i+1]))
		h.GetXaxis().SetTitle("(1/p^{RECO} - 1/p^{GEN}) / (1 / p^{GEN})")
		...
		latexFit1.DrawLatex(0.19, 0.84, "%d < p <%d" %(mrange[i],mrange[i+1]))
		...
		saveas = "/PRes_P%d_%d_%s_%s" %(mrange[i],mrange[i+1],rap,fit)
...
#        (pars2,_,_) = doFitGeneric(hist,output,rapidity,"crystal")
	...
	for k,f in enumerate(param): 
		...
		f.SetFillColor(0)
		f.GetXaxis().SetTitle("p_{T}^{#mu} [GeV]")
...
#                fun.FixParameter(2,0.)
		...
		c2.SetGrid()
		saveas = "/%sVsPt_%s_%s" %(f.GetName(),trackType,rapidity)
...
def makeMassRes(inputfile,output,funct,trackType,weights):
	...
	resB.SetMarkerStyle(20)
	resB.SetMarkerSize(1)
	resB.SetMarkerColor(ROOT.kRed)
	resB.SetLineColor(ROOT.kRed)
	resB.SetFillColor(0)
	resB.SetTitle("Muon p resolution vs p")
	resB.GetYaxis().SetTitle("Muon p Resolution")
#    resBB.GetYaxis().SetTitleOffset(1.5)
	...
	resO.SetMarkerSize(1.0)
	resO.SetMarkerColor(ROOT.kGreen+1)
	resO.SetLineColor(ROOT.kGreen+1)
	resO.SetFillColor(0)
	resO.SetTitle("Muon p_{T} resolution vs p")
	resO.GetYaxis().SetTitle("Muon p Resolution")
	resO.GetYaxis().SetTitleOffset(1.5)
 #   resBE.GetXaxis().SetTitle("m(#mu^{+}#mu^{-}) [GeV]")
	...
	resE.SetMarkerColor(ROOT.kBlue)
	resE.SetLineColor(ROOT.kBlue)
	resE.SetFillColor(0)
	resE.SetTitle("Muon p resolution vs p")
	resE.GetYaxis().SetTitle("Muon p Resolution")
	resE.GetYaxis().SetTitleOffset(1.5)
 #   resBE.GetXaxis().SetTitle("m(#mu^{+}#mu^{-}) [GeV]")
 ...
 #   resBE.GetXaxis().SetTitle("m(#mu^{+}#mu^{-}) [GeV]")
	...
	latexFitB.SetTextSize(0.030)
	latexFitB.SetNDC(True)        
	latexFitB.SetTextColor(ROOT.kRed)

	latexFitO = ROOT.TLatex()
	latexFitO.SetTextFont(42)
	latexFitO.SetTextSize(0.030)
	latexFitO.SetNDC(True)        
	latexFitO.SetTextColor(ROOT.kGreen+2)
	
	latexFitE = ROOT.TLatex()
	latexFitE.SetTextFont(42)
	latexFitE.SetTextSize(0.030)
...
#~ 
	...
	res.SetGrid()
	saveas = "/PResolutionVsP_%s"%trackType 
